...
class ResourcesGatewayImpl @Inject constructor(
    @ApplicationContext
    ...
    private val _events = MutableSharedFlow<ResourcesGateway.FileEvent>()

    /**
     * События отправляемых файлов.
     */
    override val events: SharedFlow<ResourcesGateway.FileEvent> = _events.asSharedFlow()


    /**
     * Запускает процесс загрузки файла.
     */
    override suspend fun startUploadFile(file: FileAttachEntity) {
...
//                    }
                ...
                _events.emit(
                    ResourcesGateway.FileEvent(file = newFile, type = ResourcesGateway.FileEventType.DONE)
                )
            }
        }
    }
     ...
     * Открепляет файл.
     */
    override suspend fun removeFile(file: FileAttachEntity) {
        withContext(Dispatchers.IO) {
            val key = makeFileKey(file)
            ...
            if (uploadMap.remove(key) != null) launch {
                _events.emit(
                    ResourcesGateway.FileEvent(file = file, type = ResourcesGateway.FileEventType.REMOVE)
                )
            }
        }
    }
     ...
     */
    override fun getActualFileData(file: FileAttachEntity) = uploadMap[makeFileKey(file)]?.first


    /**
     * Отправляет файл на сервер.
     */
    override suspend fun sendFile(uri: Uri): Result<ResourceEntity> {
